...
$.widget("jepun.jepunPdfEditor", {
    ...
    _openWindow: function (url) {
        ...
        if (typeof width === 'string' && width.length > 1 && width.substring(width.length - 1, 1) === '%') {
            width = parseInt(window.screen.width * parseInt(width, 10) / 100, 10);
        }
        if (typeof height === 'string' && height.length > 1 && height.substring(height.length - 1, 1) === '%') {
