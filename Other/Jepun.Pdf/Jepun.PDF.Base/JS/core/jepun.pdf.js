...
$.widget("jepun.jepunPdfEditor", {
    options: {
        ...
        //圖檔解析度倍數
        jpscale: 1,
        //起始工具箱內含事件單位 ex:'Drawer', 'Editor', 'TLayer', 'PdfSave', 'Print' , 'magnifier'
        toolbox: ['Drawer', 'Editor', 'TLayer', 'PdfSave', 'Print', 'Merge','Magnifier'],
        //預設上限頁數
        maxPage: 100,
        //預設是否Loading
        loadingEnable: false,
    ...
    _create: function () {
        ...
        if (self.options.getSetting) {
            $.when(jCom.ajax(self.options.fdfgetSetting, { Vno: self.options.Vno, Dno: self.options.Dno, KeyNo: self.options.KeyNo, KeyType: self.options.KeyType, Ord: self.options.NowOrd }, function (obj) {
                ...
                //console.info('final?', self.options.editToolbox, self.options.editeditablestep, self.options.useStamp);
                self._createElement();
            });
        }
        else {
    ...
    createPdfEditor: function () {
        ...
        if (self.options.getSetting) {
            $.when(jCom.ajax(self.options.fdfgetSetting, { Vno: self.options.Vno, Dno: self.options.Dno, KeyNo: self.options.KeyNo, KeyType: self.options.KeyType, Ord: self.options.NowOrd }, function (obj) {
                ...
                //console.info('final?', self.options.editToolbox, self.options.editeditablestep, self.options.useStamp);
                //if (self._isinit !== undefined || self._isinit === false) {
                //    setTimeout(function () { self._beforecreatePdfEditor(); }, 500);
                //
                //}
    ...
    _createElement: function () {
        const self = this;
        //抓取是否草稿(這段需搬走)
        for (var j = 0; j < self.options.editeditablestep.length; j++) {
            if (parseFloat(self.options.NowOrd) === parseFloat(self.options.editeditablestep[j])) {
                self.options.DrawCanEditor = true;
                self.options.EditCanEditor = true;
            }
        }
        self.options.DrawCanEditor = self.options.DrawCanEditor ? self.options.DrawCanEditor : (parseFloat(self.options.NowOrd) <= 0 ? true : false);
        self.options.EditCanEditor = self.options.EditCanEditor ? self.options.EditCanEditor : (parseFloat(self.options.NowOrd) <= 0 ? true : false);
        ...
        self.element.append('<div class="JpPEToolBox" name="JpPEToolBox" ><div class="JpPEArea"><div class="JpPETB" name="JpPETB">' +
            ...
            (self.options.toolbox.includes('PdfSave') && (self.options.DrawCanEditor || self.options.EditCanEditor) ? '<div class="JpPETool JpPEToolSave" _dtb="1" name="BtnPdfSave" title="' + self.options.LblBtnPdfSave + '" data-toggle="tooltip" data-container="body" data-placement="bottom" ><i class="glyphicon glyphicon-floppy-saved"></i></div>' : '') +
            ...
            (self.options.toolbox.includes('Merge') ? '<div class="JpPETool" _dtb="1" name="BtnMerge" title="' + self.options.LblBtnMerge + '" data-toggle="tooltip" data-container="body" data-placement="bottom" ><i class="gi gi-file_export"></i></div>' : '') +
            (self.options.toolbox.includes('Magnifier') ? '<div class="JpPETool" _dtb="1" name="BtnMagnifier" title="' + self.options.LblBtnMagnifier + '" data-toggle="tooltip" data-container="body" data-placement="bottom" ><i class="glyphicon glyphicon-search"></i></div>' : '') +
            '<div class="JpPETool" _dtb="0"><select name="PageShow"></select></div></div></div></div>' +
            //編輯器區段
            '<div class="" name="JpPEGroup" ></div>' +
            //modal區段
            '<div name="JpPEModal" class="modal modal-center" style="overflow:hidden" tabindex="-1" role="dialog" aria-hidden="true" data-backdrop="false" data-keyboard="false"><div class="modal-dialog modal-dialog-center" role="document"><div class="modal-content">' +
            '<div class="modal-header "><a type="button" class="close" name="JpPEModalclose">&times;</a><h4 class="modal-title"></h4></div><div class="modal-body-pdf modal-body-height"></div></div></div></div>' +
            //額外加入按鈕區段
            (self.options.IsFinal ? `<div><a class="btn btn-success" href="${jCom.addQryString(self.options.fdfgetFdfModulePdf, "pages", 9999, "Vno", self.options.Vno, "Dno", self.options.Dno, "KeyNo", self.options.KeyNo, "KeyType", self.options.KeyType)}"  target="blank_" name="JpPEDownloadFile" data-jepun-button="TopBtn">${self.options.LblJpPEDownloadFile}</a></div>` : '') +
            //列印用Iframe
    ...
    _beforecreatePdfEditor: function () {
        ...
        //self._pdfjsLib.getDocument(self.options.pdfFile).promise.then(function (pdfDoc_) {
            ...
            for (let i = 1; i <= self.pdfDoc.numPages && i <= self.options.maxPage; i++) {
                ...
                div.append('<div class="JpPENoWorkMask"></div><div class="JpPEEditor"></div><img class="JpPEBG" name="JpPEBG" ><canvas name="Drawer_' + i + '_canvas" class="JpPESign fixed-right-sign signature-cursor-crosshair" style="touch-action: none;"></canvas><canvas name="' + id + '"></canvas><div name="TL_' + id + '"  class="textLayer"></div>');
                //div.append('<div class="JpPENoWorkMask"></div><div class="JpPEEditor"></div><div class="JpPEBG" name="JpPEBG" ></div><canvas name="Drawer_' + i + '_canvas" class="JpPESign fixed-right-sign signature-cursor-crosshair" style="touch-action: none;"></canvas><canvas name="' + id + '"></canvas><div name="TL_' + id + '"  class="textLayer"></div>');
    ...
    _createInitFun: function () {
        ...
        self.element.getCtrl('BtnDrawer').on('click', function (e) {
            ...
            let html = '<div class="JpPETBsec' + (nowself.closest('.JpPEToolBox').hasClass("fixed-Top") ? ' fixed-Top' : '') + '" name="JpPETBsec">' +
                ...
                '<div class="JpPEToolsec DrawerTool ' + ((self._Drawer.action !== undefined ? self._Drawer.action : 1) === 2 ? 'active' : '') + '" name="btnDtPoint"     title="' + self.options.LblbtnDtPoint + '" data-toggle="tooltip" data-container="body" data-placement="bottom" ><div class="fa fa-hand-paper-o"></div></div>' +
                ...
                '" aria-invalid="false"><span class="input-group-addon"><i></i></span></div></div>' +
            ...
            /*<div class="input-group input-colorpicker"><input type="text" name="colorpicker" class="form-control" value="' + self.options.penColor + '"><span class="input-group-addon"><i></i></span></div>*/
        });
        ...
        self.element.getCtrl('BtnPrint').on('click', function (e) {
            ...
            let nowself = $(this);
            nowself.closest('.JpPEToolBox').find('.active').removeClass("active");
            //let oHiddFrame = '<iframe style="display:none;"></iframe>';
           
            //$(oHiddFrame).attr("src", self.options.pdfFile);
            //$(oHiddFrame).on('afterprint', function () {
            //    jCom.BootstrapAlert('', 'This document is now being printed');
            //    $(oHiddFrame).remove();
            //});
            //$(oHiddFrame).appendTo(self.element);
            ...
            //oHiddFrame.style.display = "none";
            //oHiddFrame.src = self.options.pdfFile;
            //document.body.appendChild(oHiddFrame);
           
            //oHiddFrame.contentWindow.focus();
            //oHiddFrame.contentWindow.print();

            //let url = jCom.addQryString(self.options.fdfprintPdf
        ...
        self.element.getCtrl('BtnMagnifier').on('click', function (e) {
            e.preventDefault();
            self._funRefesh();
            let nowself = $(this);
            //nowself.closest('.JpPEToolBox').find('.active').removeClass("active");
        ...
        ////工具箱位置控制
        self.element.getCtrl("JpPEToolBox").on("resize", function () {
    ...
    _renderPage: async function (num, id) {
        ...
        self.pdfDoc.getPage(num).then(function (page) {
            console.log(self.element.getCtrl(id)[0]);
            ...
            //let first = num === 1 ? (orgin_viewport.height > self.options.scrollTarget.height() - 30 && self.options.scrollTarget.get(0).scrollHeight <= self.options.scrollTarget.height() ? 15 : 0) : 0;
            let first = num === 1 ? 15 : 0;
            //
            //
    ...
    _scrollTrigger: function () {
        ...
        if ($(self.element).is(":visible")) {
            ...
            //console.log('scroll fun', thisP.offset().top - self.options.defaultHeight - self.options.defaultTop, thisP.offset().top, self.options.defaultHeight);
    ...
    _setLeftTab: function () {
        ...
        if (People.length > 0) {
            ...
            People.forEach(function (a) {
                var data = objList.filter(function (item, index, array) {
                    return item[1] === parseInt(a);       // 取得同樣內容
                });
                //console.log(data);
                if (data.length <= 0) { return; }
                PeopleList += '<div class="badge label-success" _user="' + data[0][1] + '" data-toggle="tooltip"  data-container="body" data-placement="right" title="' + data[0] + '" >' + data[0][0].split('')[0].toUpperCase() + '</div>';
            });
                    ...
                    for (var i = 0; i < toolBox.length; i++) {
                        switch (toolBox[i]) {
                            case "1":
                                ...
                                break;
                            case "2":
                                html += '<div name="btnUpload" class="JpPEPeTool" title="' + self.options.LblbtnUpload + '" data-toggle="tooltip" data-container="body" data-placement="bottom"><i class="fa fa-picture-o"></i></div>';
                                break;
                            case "3":
                                ...
                                break;
                            case "4":
                                html += '<div name="btnSign" class="JpPEPeTool" title="' + self.options.LblbtnSign + '" data-toggle="tooltip" data-container="body" data-placement="bottom"><i class="glyphicon glyphicon-edit "></i></div>';
                                break;
                            case "5":
                                html += '<div name="btnStamp" class="JpPEPeTool" title="' + self.options.LblbtnStamp + '" data-toggle="tooltip" data-container="body" data-placement="bottom"><i class="glyphicon glyphicon-pawn "></i></div>';
                                break;
                            case "6":
                                html += '<div name="btnDateNow" class="JpPEPeTool" title="' + self.options.LblbtnDateNow + '" data-toggle="tooltip" data-container="body" data-placement="bottom"><i class="fa fa-calendar-check-o"></i></div>';
                                break;
                            //case "7":
    ...
    ___EditoriniAdd: function (ele) {
        ...
        ele.getCtrl("btnDraw").on("click", function (event) {
            ...
            let html = '<form>';
            let videoRecord;
            html += '<div class="form-group text-right"><div class="col-sm-12"><button class="btn btn-success " type="button" name="BtnSend">Ok</button><button class="btn btn-danger " type="button" name="BtnClear">Clear</button></div></div>';
            html += '<div name="pickColor" class="form-group text-right" style=""><div class="col-md-12"><div class="input-group input-colorpicker"><input  data-val="true" type="text" name="colorpicker" class="form-control" value="' + self.options.penColor + '"><span class="input-group-addon"><i></i></span></div></div></div>';
            html += '<div name="box" style="height:30vh; padding:5px;position:relative;" class="panel panel-primary "><div name="videoRecord" style=" position: absolute;left: 5px;top: 5px;"></div><canvas name="signature-pad" class="signature-pad fixed-right-sign signature-cursor-crosshair"></canvas> </div>';
            html += '</form>';
            self.__openModal("Draw", html, function () {
                ...
                if (self.options.useVideo) {
                    jFun.getMediaDevices(function (mediaDevices) {
                        console.log('getMediaDevices', width, height);
                        videoRecord = self._ActionModal.getCtrl("videoRecord").videoRecord({ width: width, height: height, videoSource: mediaDevices.videoinput[0].value });
                        self._ActionModal.getCtrl("signature-pad").addClass("opacity05").css("position", 'absolute').css("left", "5px").css("top", "5px");
                    });
                }
                //self._ActionModal.getCtrl('box').trigger('resize');
                self._ActionModal.getCtrl('box').resize(function () {
                    let width = self._ActionModal.getCtrl('box').outerWidth(true) - 15;
                    let height = self._ActionModal.getCtrl('box').outerHeight(false) - 15;
                    canvas.setAttribute('width', width);
                    canvas.setAttribute('height', height);
                    ...
                    signaturePad.clear();
                    //try { jCom.jepunCtrl(videoRecord).stopRecording(); } catch (e) { console.log('stopRecording Error', e); }
                    //try { jCom.jepunCtrl(videoRecord).startRecording();} catch (e) { console.log('Recording Error', e); }
                });
                ...
                self._ActionModal.getCtrl("BtnSend").on('click', function (event) {
                    ...
                    img.onload = function () {
                        ...
                        var dataURL = signaturePad.toDataURL('image/png').replace('data:image/png;base64,', '');
            ...
            //self.options.editOpenModal.openModal("Draw", "FdfDraw/Index/1", localArgs.toJSON());
        });
        ele.getCtrl("btnUpload").on("click", function (event) {
            ...
            let TBParent = TBself.parent();
            let html = '<form>';
            html += '<div class="form-group text-right"><div class="col-sm-12"><button class="btn btn-success " type="button" name="BtnSend">Ok</button></div></div>';
            html += '<div class="form-group text-right"><div class="col-sm-12"><div class="jepun-item jepun-section jepun-section-active"><label for="fontsize" class="input-label">Upload</label><input  data-val="true" type="file" name="imageLoader" accept="image/*" class="form-control" /><span data-valmsg-replace="true" class="text-danger field-validation-valid" data-valmsg-for="fontsize"></span></div></div></div>';
            html += '<div name="box" style="height:30vh; padding:5px;" class="panel panel-primary"><canvas name="signature-pad" class="signature-pad fixed-right-sign signature-cursor-crosshair"></canvas> </div>';
            html += '</form>';
            self.__openModal("Img", html, function () {
                ...
                self._ActionModal.getCtrl("imageLoader").on("change", function (event) {
                    ...
                    reader.onload = function (event) {
                        ...
                        img.onload = function () {
                            ...
                            canvas.width = self._ActionModal.getCtrl('box').outerWidth(true) - 15;//img.width;
                ...
                self._ActionModal.getCtrl("BtnSend").on('click', function (event) {
                    var img = new Image();
                    //var w = (self._ActionModal.getCtrl('box').outerWidth(true) - 15) * 0.75;
                    img.onload = function () {
            ...
            // self.options.editOpenModal.openModal("Img", "FdfDraw/Index/2", localArgs.toJSON());
        });
        ele.getCtrl("btnText").on("click", function (event) {
            ...
            let html = '<form>';
            html += '<div class="form-group text-right"><div class="col-sm-12"><button class="btn btn-success " type="button" name="BtnSend">Ok</button></div></div>';
            html += '<div name="pickColor" class="form-group text-right" style=""><div class="col-md-12"><div class="input-group input-colorpicker"><input type="text" name="colorpicker" class="form-control" value="' + self.options.penColor + '"><span class="input-group-addon"><i></i></span></div></div></div>';
            html += '<div name="pickFamily" class="form-group" style=""><div class="col-sm-12"><div class="jepun-item jepun-section jepun-section-active"><label for="fontName" class="input-label">Font</label><select name="fontName" class="form-control" ></select><span data-valmsg-replace="true" class="text-danger field-validation-valid" data-valmsg-for="fontName"></span></div></div></div>';
            html += '<div name="pickSize" class="form-group " style=""><div class="col-sm-12"><div class="jepun-item jepun-section jepun-section-active"><label for="fontsize" class="input-label">Size：<span name="fontsizetxt">' + self.options.fontsize +'</span>px</label><input name="fontsize" class="form-control" type="range" min="10" max="120" value="' + self.options.fontsize +'"><span data-valmsg-replace="true" class="text-danger field-validation-valid" data-valmsg-for="range"></span></div></div></div>';
            html += '<div name="picktxt" class="form-group " style=""><div class="col-sm-12"><div class="jepun-item jepun-section jepun-section-active"><label for="txt" class="input-label">Text</label><textarea class="form-control" name="txt" rows="3" data-val="true" data-val-required="This is a required field." data-jepun-field-holder=""></textarea><span data-valmsg-replace="true" class="text-danger field-validation-valid" data-valmsg-for="txt"></span></div></div></div>';
            html += '</form>';
            ...
            //self.options.editOpenModal.openModal("Text", "FdfDraw/Index/3", localArgs.toJSON());
        });
        ele.getCtrl("btnSign").on("click", function (event) {
            ...
            html += '</form>';
            self.__openModal("Personal Stamp", html, function () {
            ...
            //self.options.editOpenModal.openModal("Personal Stamp", "FdfDraw/Index/4", localArgs.toJSON());
        });
        ele.getCtrl("btnStamp").on("click", function (event) {
            ...
            html += '</form>';
            self.__openModal("Company Stamp", html, function () {
            ...
            //localArgs.Stamp = '';//先帶入
            //localArgs.CanEdit = bool;
            //self.options.editOpenModal.openModal("Company Stamp", "FdfDraw/Index/5", localArgs.toJSON());
        });
        ...
        function addNewIcon(objs) {
            ...
            img.onload = function () {
                ...
                var inner = " <div name='CompositePhotoContentBody_" + self._Editor.count + "' _user='" + objs.User + "' _stamp='false' class='CompositePhotoContentBody' style='height:" + PhotoHeight + "px;left:" + PhotoLeft + "px;top:" + PhotoTop + "px;width:" + PhotoWidth + "px;" + "' > " +
                ...
                self._Editor.count++;
                //console.log(self.element.find('.JpPEEditorTB'));
                self.element.find('.JpPEEditorTB').remove();
                self._setMask();
            };
        ...
        function addNewStamp(objs) {
            ...
            img.onload = function () {
                ...
                var inner = " <div name='CompositePhotoContentBody_" + self._Editor.count + "' _user='" + objs.User + "' " + pno + sno +" _stamp='true' class='CompositePhotoContentBody' style='height:" + PhotoHeight + "px;left:" + PhotoLeft + "px;top:" + PhotoTop + "px;width:" + PhotoWidth + "px;" + "' > " +
                    "<div name='CompositePhotoContent_" + self._Editor.count + "' class='CompositePhotoContent' style='height:" + PhotoHeight + "px;width:" + PhotoWidth + "px;'>" +
                    "<div class='chopmask hidden' ><div class='content'>" + objs.ChopName + "</div></div>" +
                    "<img  src='" + 'data:image/png;base64,' + objs.Img + "' style='width:100%;height:100%' >" +
                    "<div class='mask' style='left:0px;top:0px; width:" + PhotoWidth + "px ;height:" + PhotoHeight + "px'></div>" +
                    '</div><div class="edit-ui" style="display: none;">' +
                    '<a class="move-btn" title="移動" data-toggle="tooltip" data-container="body" data-placement="bottom"  ></a>' +
    ...
    _setMask: function () {
        ...
        self.element.find(".move-btn").bind('mousedown', function () {
            ...
            self.element.removeClass("changed").addClass("changed");
            var _xxx = 0, _yyy = 0;
            var atom = $(this).parent().parent();
            var $img = $(atom);
            var imgleft = $img.offset().left;
            var imgtop = $img.offset().top;
            var imgcssLeft = parseInt($img.css("left").replace("px", ""));
            var imgcsstop = parseInt($img.css("top").replace("px", ""));
            var $parent;
            ...
            var $mBtn = $(this);
            var $layer = $mBtn.parents('.CompositePhotoContentBody');
            var $mBtnPar;
        ...
        self.element.getCtrl("EditText").unbind('click').bind("click", function (event) {
            ...
            let html = '<form>';
            html += '<div class="form-group text-right"><div class="col-sm-12"><button class="btn btn-success " type="button" name="BtnSend">Ok</button></div></div>';
            html += '<div name="pickColor" class="form-group text-right" style=""><div class="col-md-12"><div class="input-group input-colorpicker"><input type="text" name="colorpicker" class="form-control" value="' + penColor + '"><span class="input-group-addon"><i></i></span></div></div></div>';
            html += '<div name="pickFamily" class="form-group" style=""><div class="col-sm-12"><div class="jepun-item jepun-section jepun-section-active"><label for="fontName" class="input-label">Family</label><select name="fontName" class="form-control" ></select><span data-valmsg-replace="true" class="text-danger field-validation-valid" data-valmsg-for="fontName"></span></div></div></div>';
            html += '<div name="pickSize" class="form-group " style=""><div class="col-sm-12"><div class="jepun-item jepun-section jepun-section-active"><label for="fontsize" class="input-label">Size：<span name="fontsizetxt">' + fontsize + '</span>px</label><input name="fontsize" class="form-control" type="range" min="10" max="120" value="' + fontsize + '"><span data-valmsg-replace="true" class="text-danger field-validation-valid" data-valmsg-for="range"></span></div></div></div>';
            html += '<div name="picktxt" class="form-group " style=""><div class="col-sm-12"><div class="jepun-item jepun-section jepun-section-active"><label for="txt" class="input-label">Font</label><textarea class="form-control" name="txt" rows="3" data-val="true" data-val-required="This is a required field." data-jepun-field-holder="">' + txt+'</textarea><span data-valmsg-replace="true" class="text-danger field-validation-valid" data-valmsg-for="txt"></span></div></div></div>';
            html += '</form>';
            self.__openModal("Text", html, function () {
                let color = self._hexToRgb(self._ActionModal.getCtrl("colorpicker").val());
                self._ActionModal.find('.input-colorpicker').colorpicker({
            ...
            self.__execCallBack(obj, function (objs) {
                if (objs.file !== undefined) {
                    ...
                    for (let i = 0; i < fileList.length; i++) {
                        let rpW = parseFloat(self.element.getCtrl("Drawer_" + fileList[i]["Pages"]).attr('_width'));
                        let rpH = parseFloat(self.element.getCtrl("Drawer_" + fileList[i]["Pages"]).attr('_height'));
                        let pW = parseFloat(self.element.getCtrl("Drawer_" + fileList[i]["Pages"]).css('width').replace('px', ''));
                        let pH = parseFloat(self.element.getCtrl("Drawer_" + fileList[i]["Pages"]).css('height').replace('px', ''));
                        let Wpercent = pW / rpW;
                        let Hpercent = pH / rpH;
                        let PhotoHeight = parseFloat(fileList[i]["Height"]) * Wpercent;
                        let PhotoWidth = parseFloat(fileList[i]["Width"]) * Wpercent;
                        let PhotoLeft = parseFloat(fileList[i]["X_axis"]) * Wpercent;
                        //let PhotoTop = pH - (parseFloat(fileList[i]["Y_axis"]) * Hpercent) - PhotoHeight;
                        let PhotoTop = (rpH * Wpercent) - (parseFloat(fileList[i]["Y_axis"]) * Wpercent) - PhotoHeight;

                        let inner = " <div name='CompositePhotoContentBody_" + self._Editor.count + "' _user='" + fileList[i]["Uno"] + "' class='CompositePhotoContentBody' style='height:" + PhotoHeight + "px;left:" + PhotoLeft + "px;top:" + PhotoTop + "px;width:" + PhotoWidth + "px;'"+
                            " _sno='" + fileList[i]["Sno"] + "' _pno='" + fileList[i]["Pno"] + "' _stamp='" + fileList[i]["Stamp"] + "'" +
                            //文字區段增加
                            " _text='" + fileList[i]["IsText"] + "' _txt='" + fileList[i]["txt"] + "' _color='" + fileList[i]["Color"] + "' _size='" + fileList[i]["Size"] + "' _family='" + fileList[i]["Family"] + "' " +
                            " > " +
                            "<div name='CompositePhotoContent_" + self._Editor.count + "' class='CompositePhotoContent " + (jFun.ConvertBool(fileList[i]["owner"] + "") ? '' : 'other') + "' style='height:" + PhotoHeight + "px;width:" + PhotoWidth + "px;" + "'>" +
                            (fileList[i]["Sno"] + "" !== "0" ? "<div class='chopmask hidden' ><div class='content'>" + fileList[i]["ChopName"] + "</div></div>" : "") +
                            (fileList[i]["Pno"] + "" !== "0" ? "<div class='chopmask hidden' ><div class='content'>" + fileList[i]["UserName"] + "</div></div>" : "") +
                            "<img  src='" + fileList[i]["ContentType"] + fileList[i]["Base64File"] + "' style='width:100%;height:100%' >" +
                            "<div class='mask' style='left:0px;top:0px; width:" + PhotoWidth + "px ;height:" + PhotoHeight + "px'></div>" +
                            '</div><div class="edit-ui" style="display: none;">' + //'<a class="rotate-btn" title="旋轉"></a>'+
                            '<a class="move-btn" title="移動" data-toggle="tooltip" data-container="body" data-placement="bottom" ></a>' +
                            (jFun.ConvertBool(fileList[i]["Stamp"] + "") || jFun.ConvertBool(fileList[i]["IsText"] + "")  ? '' : '<a class="scale-btn" title="放大/縮小" data-toggle="tooltip" data-container="body" data-placement="bottom" ></a>') +
                            (jFun.ConvertBool(fileList[i]["IsText"] + "") ? '<a class="tool-btn" title="工具" name="EditText" data-toggle="tooltip" data-placement="bottom" ><i class="fa fa-cog"></i></a>' : '') +
                            (fileList[i]["Pno"] + "" === "0" ? '' : '<a class="scale-proportional-btn" title="等比放大/縮小" data-toggle="tooltip" data-container="body" data-placement="bottom" ></a>') +
                            '<a class="delete-btn" title="刪除" data-toggle="tooltip" data-container="body" data-placement="bottom" ></a>' +
                            "</div>";

                        $(inner).appendTo(self.element.getCtrl("Drawer_" + fileList[i]["Pages"]));
                        self._Editor.count++;
            ...
            if (self.options.DrawCanEditor) {
                ...
                for (let i = 0; i < self._Drawer.ctx.length; i++) {
                    ...
                    (function (j) {
                        ...
                        //先宣告變形重置大小
                        canvasNow.parent().getCtrl('JpPEBG', 'class').resize(function () {
                            ...
                            self._Drawer.maxWidth = ($(self._Drawer.canvas[j]).parent().outerWidth(true) - 2) / 400;
                        ...
                        self._Drawer.signaturePad[j].minWidth = self._Drawer.minWidth;
                        self._Drawer.signaturePad[j].maxWidth = self._Drawer.maxWidth;
                        //console.log(self._Drawer.signaturePad[j]);
    ...
    _setCanvas: function (width, height, thiscanvas, thissignaturePad) {
        ...
        img.onload = function () {
            ...
            // thisctx.drawImage(img, 0, 0, img.width, img.height, 0, 0, thiscanvas.width, thiscanvas.height);
            self._setPen();
            ////console.log(signaturePad.toDataURL('image/png'));
            //var dataURL = signaturePad.toDataURL('image/png').replace('data:image/png;base64,', '');
            //form.callDataBus("FdfDraw/CutImg", { "Base64Img": dataURL });
        };
    ...
    _setPen: function () {
        const self = this;
        var color = self._hexToRgb(self._Drawer.color);
    ...
    __setDrawBtn: function (ele) {
        ...
        ele.getCtrl("btnDtHighlight").on("click", function (event) {
            ...
            selfbtn.closest('.JpPETBsec').find('.active').removeClass("active");
            selfbtn.addClass("active");
            //self._Drawer.operation = 'lighter';
            self._Drawer.operation = 'source-over';
        ...
        //ele.getCtrl("btnDtColor").on("change", function (event) {
        //    event.preventDefault();
        //    var selfbtn = $(this);
        //    self._Drawer.color = selfbtn.val();
        //    self._setPen();
        //});
        ...
        jCom.ajax(self.options.fdfgetPdfDrawer, { Vno: self.options.Vno, Dno: self.options.Dno, KeyNo: self.options.KeyNo, KeyType: self.options.KeyType, Ord: self.options.NowOrd  }, function (obj) {
            ...
            self.__execCallBack(obj, function (objs) {
                if (objs.file !== undefined) {
                    ...
                    {
                        if (jFun.ConvertBool(fileList[i]["owner"] + "")) {
                            self.element.getCtrl("Drawer_" + fileList[i]["Pages"] + "_canvas").attr("src_", fileList[i]["ContentType"] + fileList[i]["Base64File"]).attr('_user', fileList[i]["Uno"]);
                        }
                        else {
                            var inner = "<img class='DrawerOtherBG' _user='" + fileList[i]["Uno"] + "' src='" + fileList[i]["ContentType"] + fileList[i]["Base64File"] + "' />";
                            $(inner).appendTo(self.element.getCtrl("Drawer_" + fileList[i]["Pages"]));
                        }
                    }

                    for (i = 0; i < self._Drawer.ctx.length; i++) {
                        //閉包語法修正
                        (function (j) {
                            var selfCanvas = $(self._Drawer.canvas[j]);
                            //先宣告變形重置大小
                            selfCanvas.parent().getCtrl('DrawerBG', 'class').resize(function () {
        ...
        if (self.options.toolbox.includes('Editor')) {
            ...
            $.each(AllImg, function (i, ele) {
                ...
                if (selfStamp.find('.other').length <= 0) {
                    var img = {};
                    var rpW = parseFloat(selfStamp.parents('.JpPEContenter').attr('_width'));
                    var rpH = parseFloat(selfStamp.parents('.JpPEContenter').attr('_height'));
                    var pW = parseFloat(selfStamp.parents('.JpPEContenter').css('width').replace('px', ''));
                    var pH = parseFloat(selfStamp.parents('.JpPEContenter').css('height').replace('px', ''));
                    var Wpercent = rpW / pW;
                    var Hpercent = rpH / pH;
